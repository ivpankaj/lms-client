...
import {semestersData } from './MyCoursesSemesterData'; // Ensure correct import path
// import {semestersData} from './MyCoursesSemesterData2'
...
const SemesterView = () => {
  ...
  const [modules, setModules] = useState(semestersData[currentSemester]);
  ...
  useEffect(() => {
    setModules(semestersData[currentSemester]); // Replace this with API call if needed
  }, [currentSemester]);
  ...
  const handleNext = () => {
    if (currentSemester < Object.keys(semestersData).length) setCurrentSemester(currentSemester + 1);
  };
    ...
    <Layout>
      <div className="w-screen max-w-screen-lg mx-auto p-4 sm:p-6 lg:p-8 bg-gray-100 rounded-lg shadow-lg overflow-x-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between mt-6 space-y-2 sm:space-y-0">
          ...
          <button
            className={`px-4 py-2 rounded-lg flex-1 sm:flex-initial text-xs sm:text-sm md:text-base ${
              currentSemester === Object.keys(semestersData).length
                ? 'bg-gray-300 text-gray-600 cursor-not-allowed'
                : 'bg-blue-600 text-white'
            }`}
            disabled={currentSemester === Object.keys(semestersData).length}
            onClick={() => {
