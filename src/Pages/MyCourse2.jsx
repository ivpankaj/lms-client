...
// import { dm_basic_semesterData } from './MyCoursesSemesterData'; // Ensure correct import path
import {dm_basic_semesterData} from './MyCoursesSemesterData2'
...
const DigitalMarketing = () => {
  ...
  const [modules, setModules] = useState(dm_basic_semesterData[currentSemester]);
  ...
  useEffect(() => {
    setModules(dm_basic_semesterData[currentSemester]); // Replace this with API call if needed
  }, [currentSemester]);
  ...
  const handleNext = () => {
    if (currentSemester < Object.keys(dm_basic_semesterData).length) setCurrentSemester(currentSemester + 1);
  };
      ...
      <div className="w-screen max-w-screen-lg mx-auto p-4 sm:p-6 lg:p-8 bg-gray-100 rounded-lg shadow-lg overflow-x-auto">
        ...
        <div className="overflow-x-auto mb-6">
          <div className="flex flex-nowrap space-x-2 overflow-x-auto">
            {Object.keys(dm_basic_semesterData).map((sem) => (
              <button
                key={sem}
        ...
        <div className="flex flex-col sm:flex-row justify-between mt-6 space-y-2 sm:space-y-0">
          ...
          <button
            className={`px-4 py-2 rounded-lg flex-1 sm:flex-initial text-xs sm:text-sm md:text-base ${
              currentSemester === Object.keys(dm_basic_semesterData).length
                ? 'bg-gray-300 text-gray-600 cursor-not-allowed'
                : 'bg-blue-600 text-white'
            }`}
            disabled={currentSemester === Object.keys(dm_basic_semesterData).length}
            onClick={() => {
